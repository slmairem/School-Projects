...
void check_arrival_time(ReadyQueue *queue, Process *processes, int time, int num_processes) {
    ...
    */
    ...
    for(i ; i<num_processes ; i++){
        if(!processes[i].is_arrived && processes[i].arrival_time <= time){
            enqueue(queue, processes[i]);
        }
    }
    printf("Process %d arrived time = %d\n", processes[i].id, time);
...
void round_robin(ReadyQueue *queue, Process *processes, int quantum, int num_processes) {
    ...
    */
    ...
    while(finished_processes < num_processes){
        if(!is_empty(queue)){
            Process cur_process = dequeue(queue);

            if(cur_process.remaining_burst_time <= quantum){
                run_process(&cur_process, cur_process.remaining_burst_time, &cur_process);
...
void calculate_metrics(Process *processes, int num_processes) {
    int i= 0;
    for(i ; i < num_processes; i++) {
        /* Fill this part. Calculate TAT and WT*/ 
    ...
    /* Fill this part. Find average WT and TAT*/
