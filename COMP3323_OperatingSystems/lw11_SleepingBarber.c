...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < NUM_CUSTOMERS; i++) {
        pthread_create(&ctid[i], NULL, customer, (void*)(intptr_t)i);
        if (i % 2 == 0) {
            sleep(1);  
        }
    }
...
void* customer(void* num) {
    ...
    if(sem_trywait(&waitingChairs)==0){ 
        printf("Customer %d sits chair and waiting.\n", customerNumber);
...
//I change the sleep because of the expected output, if you change it the result changes too
void getHairCut(int customerNumber) {
    printf("Customer %d is getting a haircut.\n", customerNumber);
    sleep(2);  // Simulate the time taken for a haircut
}
